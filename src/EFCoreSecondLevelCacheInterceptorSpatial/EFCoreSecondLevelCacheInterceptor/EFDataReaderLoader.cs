...
namespace EFCoreSecondLevelCacheInterceptor
{
    ...
    public class EFDataReaderLoader : DbDataReader
    {
        ...
        private static readonly Type _sqlGeographyType = typeof(SqlGeography);
        ...
        public override bool Read()
        {
            ...
            {
                var dataTypeName = _dbReader.GetDataTypeName(i);
                if (dataTypeName.EndsWith("sys.geography"))
                {
                    // TODO: Set real data
                    rowValues.Add(new Point(0, 0));
                }
                else
                {
                    rowValues.Add(_dbReader.GetValue(i));
                }
            }

            _rowValues = rowValues.ToArray();
